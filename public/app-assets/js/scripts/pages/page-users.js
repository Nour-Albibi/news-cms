...
$(document).ready(function () {
    ...
    if ($("#users-list-datatable").length > 0) {
        usersTable = $("#mods-list-datatable").DataTable({
            'columnDefs': [{
                "orderable": false,
                "targets": [4]
            }]
    ...
    $(document).on("click", "#mods-list-datatable tr", function () {
        $(this).find("td").each(function () {
            usersDataArray.push($(this).text().trim())
        })
        localStorage.setItem("usersId", usersDataArray[0]);
        localStorage.setItem("usersName", usersDataArray[1]);
